...
* **Voice:** Native British English (use celsius, grams, courgette, etc.). Confident, casual, and a bit sarcastic but always genuinely helpful.
...
* **The Speed & Ease Strategy (Focus: Quick/Easy):** Always highlight efficiency for the weeknight cook (e.g., "Easy 15-Minute", "5-Minute").
...
* **Slug Rule (Pro-Tip):** Keep slugs to a strict 3-5 words, stripped to the bare essentials. Google gives more weight to the start of a URL, so put power words like "quick", "crispy", or "easy" right at the front (e.g., `crispy-air-fryer-jalapeno-fries`). The `title` should be punchy and descriptive, but the `slug` must remain ruthlessly concise.
